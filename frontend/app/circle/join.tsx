...
export default function JoinCircleScreen() {
  ...
  const handleJoin = async () => {
    ...
    if (!cleanCode || cleanCode.length < 4) { alert('Cod invalid'); return; }
    if (!user) { alert('Nu ești autentificat'); return; }
    setIsLoading(true);
    try {
      const { data: rpcData, error: rpcError } = await supabase.rpc('lookup_circle_by_invite_code', { p_invite_code: cleanCode });
      if (rpcError) { alert('Eroare: ' + rpcError.message); setIsLoading(false); return; }
      const circleData = Array.isArray(rpcData) && rpcData.length > 0 ? rpcData[0] : null;
      if (!circleData) { alert('Cercul nu a fost găsit'); setIsLoading(false); return; }
      const { data: existingMember } = await supabase.from('circle_members').select('*').eq('circle_id', circleData.id).eq('user_id', user.id).single();
      if (existingMember) { alert('Ești deja membru!'); setIsLoading(false); router.replace('/(main)/family'); return; }
      const { error: memberError } = await supabase.from('circle_members').insert({ circle_id: circleData.id, user_id: user.id, role: profile?.role || 'child', joined_at: new Date().toISOString() });
      if (memberError) { alert('Eroare: ' + memberError.message); setIsLoading(false); return; }
      addCircle(circleData);
      setCurrentCircle(circleData);
      const memberRole = profile?.role || 'child';
      setCurrentRole(memberRole);
      try { await AsyncStorage.setItem('@guardian_cached_role', memberRole); } catch (e) {}
      alert('Te-ai alăturat cercului!');
      const CHILD_ROLES = ['child', 'teen'];
      if (CHILD_ROLES.includes(memberRole.toLowerCase())) { router.replace('/(child)/home'); } else { router.replace('/(main)/family'); }
    } catch (error) { alert('Eroare'); } finally { setIsLoading(false); }
  };
    ...
    <SafeAreaView style={styles.container}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.keyboardView}>
        ...
        <View style={styles.content}>
          <View style={styles.iconWrapper}><Ionicons name="enter" size={48} color="#10B981" /></View>
          <Text style={styles.title}>Join Family Circle</Text>
          <Text style={styles.subtitle}>Enter the invite code</Text>
          <View style={styles.inputContainer}>
            <Text style={styles.label}>Invite Code</Text>
            <View style={styles.inputWrapper}>
              <TextInput style={styles.input} placeholder="XXXXXX" placeholderTextColor="#64748B" value={code} onChangeText={(text) => setCode(text.toUpperCase())} autoCapitalize="characters" maxLength={6} autoCorrect={false} />
            </View>
          </View>
          <TouchableOpacity style={[styles.joinButton, isLoading && styles.buttonDisabled]} onPress={handleJoin} disabled={isLoading}>
            {isLoading ? <ActivityIndicator color="#FFFFFF" /> : <><Text style={styles.joinButtonText}>Join Circle</Text><Ionicons name="arrow-forward" size={20} color="#FFFFFF" /></>}
          </TouchableOpacity>
          <View style={styles.divider}><View style={styles.dividerLine} /><Text style={styles.dividerText}>or</Text><View style={styles.dividerLine} /></View>
          <TouchableOpacity style={styles.createLink} onPress={() => router.replace('/circle/create')}><Text style={styles.createLinkText}>Create a new circle</Text></TouchableOpacity>
        </View>
